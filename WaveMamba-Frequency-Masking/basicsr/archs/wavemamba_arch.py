...
def iwt_init(x):
    ...
    out_batch, out_channel = in_batch, int(in_channel / (r ** 2))
...
class DownFRG(nn.Module):
    ...
    def forward(self, x, x_d):
        ...
        return x_LL, x_h, (x_HL, x_LH, x_HH)
...
class UNet(nn.Module):
    def __init__(self, in_chn=3, wf=48, n_l_blocks=[1, 1, 2], n_h_blocks=[1, 1, 1], ffn_scale=2, use_dwt_weight=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x1 = self.conv_01(img)
        # LFSS + HFE + SKFF
        x_l, x_H1, wavelets1 = self.down_group1(x1, img_down1)
        x_l, x_H2, wavelets2 = self.down_group2(x_l, img_down2)
        x_l, x_H3, wavelets3 = self.down_group3(x_l, img_down3)
        ...
        # Reconstruct
        I_0 = self.last(x_l) + img

        if self.use_dwt_weight:
            # after DWT: wavelet-based weight map from raw DWT components
            m_list = []
            for wavelets in [wavelets1, wavelets2, wavelets3]:
                x_HL, x_LH, x_HH = wavelets
                detail = torch.abs(x_HL) + torch.abs(x_LH) + torch.abs(x_HH)  # sum of abs coeffs
                m = detail.mean(dim=1, keepdim=True)  # single-channel
                m = F.interpolate(m, size=img.shape[-2:], mode='bilinear', align_corners=False)
                m_list.append(m)
        else:
            # after DownFRG: build weighted map M_w
            m_list = []
            for x_H in [x_H1, x_H2, x_H3]:  # DownFRG return High frequency x_h
                m = torch.abs(x_H).mean(1, keepdim=True)  # convert to single-channel map
                m = F.interpolate(m, size=img.shape[-2:], mode='bilinear', align_corners=False)  # upsample to input size
                m_list.append(m)

        # average three wavelet maps and normalize
        M_w = torch.stack(m_list, dim=0).mean(0)
        M_w = (M_w - M_w.min()) / (M_w.max() - M_w.min() + 1e-6) # + 1e-6 avoid division by zero

        I_high = M_w * img + (1.0 - M_w) * I_0
        return I_high

@ARCH_REGISTRY.register()
class WaveMamba(nn.Module):
    def __init__(self, *, in_chn, wf, n_l_blocks=[1, 1, 2], n_h_blocks=[1, 1, 1], ffn_scale=2.0, **ignore_kwargs):
        super().__init__()
        self.restoration_network = UNet(in_chn=in_chn, wf=wf, n_l_blocks=n_l_blocks, n_h_blocks=n_h_blocks,
                                        ffn_scale=ffn_scale)
    ...
    def encode_and_decode(self, input, current_iter=None):
        return self.restoration_network(input)
